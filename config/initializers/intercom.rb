IntercomRails.config do |config|
  # == Intercom app_id
  #
  config.app_id = ENV["INTERCOM_APP_ID"] || "l2sbzelx"

  # == Intercom secret key
  # This is required to enable secure mode, you can find it on your Intercom
  # "security" configuration page.
  #
  config.api_secret = "5LLuQ_g3xDskl_QrAWU4disW_zxBRABtx99ZNK9K"
  ...
  #
  config.api_key = "6c84920f3c47cf420566e0e5c869f9ffb430cfdd"
  ...
  config.user.custom_data = {
     :digest => Proc.new { |current_user| current_user.email_digest },
     :claimed => Proc.new { |current_user| current_user.claimed }
  }
  ...
  #
  config.user.company_association = Proc.new { |user| user.organizations }
  ...
  #
  # config.company.current = Proc.new { |user| user.organizations[0] }
